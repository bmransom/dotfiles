...
`dot_agents/skills`; use `dot_agents/scripts/install-skills.sh` to expose them to harness-specific
locations such as `~/.claude/skills`. Run `chezmoi status` to catch drift. To commit and push every
edit automatically, uncomment the `[git]` block in `.chezmoi.toml.tmpl`.
...
- `dot_agents/` → harness-agnostic agent skills in `~/.agents`, plus install/validation scripts.
  ...
  symlinks are generated by `dot_agents/scripts/install-skills.sh`.
