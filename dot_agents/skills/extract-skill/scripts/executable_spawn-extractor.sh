#!/usr/bin/env bash
# spawn-extractor.sh - launch a same-harness skill drafting session from a brief.
set -euo pipefail
...
main() {
  ...
  local prompt="Read $brief in full. Draft or revise the skill described there using the Agent Skills format. Keep user-scope skills harness-agnostic; keep repo-scope skills local to the repo. Preserve reusable procedure, exclude details marked non-reusable, add realistic eval prompts, and validate the skill structure before reporting back."
  local pane_cmd
  ...
  if [ -n "${TMUX:-}" ]; then
    ...
    [ "$dry_run" -eq 1 ] || "${tmux_bin[@]}" display-message "extract-skill: spawned '$slug' in background"
