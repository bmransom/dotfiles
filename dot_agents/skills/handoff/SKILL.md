...
Write a concise handoff briefing, store it in repo-local generic agent state, then spawn a successor with the same harness.

## Workflow

1. Gather ground truth with commands, never guesses:
...
2. **State (verified)**: HEAD, committed state, uncommitted caveat, last test/gate result with numbers.
3. **What happened and why**: key decisions, rationale, dead ends, gotchas.
...
7. **Done when**: commands and expected output that prove the unit is complete.

Keep it under a few thousand tokens. Prefer specific paths, commands, hashes, metrics, and file anchors over transcript summary.
...
```bash
~/.agents/skills/handoff/scripts/spawn-successor.sh "<slug>"
```

The script detects the current harness (`claude`, `codex`, or `pi`) and opens a tmux window or detached tmux session with that same harness. If tmux or harness detection is unavailable, it prints the manual command and seed prompt.
